...
const isTechnicalSkill = (value = "") => {
  ...
  if (NON_TECHNICAL_SKILL_PATTERNS.some((pattern) => pattern.test(normalizedValue))) {
    return false;
  }

  return TECHNICAL_SKILL_PATTERNS.some((pattern) => pattern.test(normalizedValue));
};
...
const analyzeSkills = (value) => {
  ...
  for (const skill of rawSkills) {
    if (isTechnicalSkill(skill)) {
      if (!validSkills.some((item) => item.toLowerCase() === skill.toLowerCase())) {
...
const normalizeDomain = (value = "") => cleanText(value).toLowerCase() || "general";
...
const normalizeQuestion = (value) => {
  ...
  return {
    ...
    difficulty: normalizeDifficulty(typeof data.difficulty === "string" ? data.difficulty : ""),
...
const buildTopicPool = ({ focusAreas, skills, round, domain }) => {
  const domainFallback =
    round === "hr" ? "communication" : round === "managerial" ? "leadership" : domain || "fundamentals";
  ...
  const primaryTopics = [...new Set([...nonCrossCuttingFocusAreas, ...skills].filter(Boolean))];

  if (primaryTopics.length > 0) {
    return primaryTopics;
  }

  const secondaryTopics = [...new Set([...crossCuttingFocusAreas].filter(Boolean))];
...
const pickTopic = ({ focusAreas, skills, round, domain, preferredTopic, previousTopics = [], questionNumber = 1 }) => {
  if (preferredTopic) {
    return preferredTopic;
  }

  const topicPool = buildTopicPool({ focusAreas, skills, round, domain });
  const normalizedPreviousTopics = previousTopics.map((topic) => cleanText(topic).toLowerCase());
  const unseenTopics = topicPool.filter((topic) => !normalizedPreviousTopics.includes(topic.toLowerCase()));
...
const getStableVariantOffset = (seed = "") => {
  ...
  return [...normalizedSeed].reduce((total, character) => total + character.charCodeAt(0), 0);
};
...
const dedupeList = (items = []) =>
  items.reduce((result, item) => {
    ...
    if (!cleanedItem || result.some((value) => value.toLowerCase() === itemKey)) {
      return result;
    }
...
const buildScenarioLabel = ({ domain, primarySkill, questionNumber, variantSeed, companyStyle }) => {
...
const buildIssueSignal = ({ focusAreas, domain, questionNumber, variantSeed }) => {
  const normalizedFocusAreas = dedupeList(focusAreas).map((area) => area.toLowerCase());
  ...
  if (normalizedFocusAreas.some((area) => area.includes("state"))) {
    return pickVariant(questionNumber, [
  ...
  if (normalizedFocusAreas.some((area) => area.includes("reliab") || area.includes("availability"))) {
    return pickVariant(questionNumber, [
...
const getQuestionStyle = ({ companyStyle, questionNumber, previousQuestions = [], variantSeed = "" }) => {
  const allowFollowUp = Number(questionNumber || 1) > 1 || previousQuestions.length > 0;
...
const getCompanyClosingClause = ({ companyStyle, difficulty, questionNumber, variantSeed }) => {
...
const getContextDetails = ({ topic, focusAreas, skills, domain, questionNumber, variantSeed, companyStyle }) => {
  const uniqueSkills = dedupeList(skills);
  const uniqueFocusAreas = dedupeList(focusAreas);
  const relatedSkills = uniqueSkills.filter((skill) => skill.toLowerCase() !== topic.toLowerCase());
  const primarySkill = relatedSkills[0] || uniqueSkills[0] || "";
  const extraFocusAreas = uniqueFocusAreas.filter((area) => area.toLowerCase() !== topic.toLowerCase());
...
const buildTechnicalQuestion = ({
  ...
  const issueSignal = contextDetails?.issueSignal || "the feature starts behaving inconsistently";
...
const buildHrQuestion = ({ companyStyle, topic, difficulty, contextDetails, questionNumber, variantSeed }) => {
  const focusClause = contextDetails?.focusClause || "";
...
module.exports = { analyzeSkills, generateInterviewQuestion, normalizeSkills, normalizeFocusAreas };
